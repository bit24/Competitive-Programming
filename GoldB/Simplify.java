...
public class Simplify {
	...
	public static boolean connectSameComponents(Edge a, Edge b) {
		if (inSameSet(a.endPointA, b.endPointA)
				&& inSameSet(a.endPointB, b.endPointB)) {
			return true;
		} else if (inSameSet(a.endPointA, b.endPointB)
				&& inSameSet(a.endPointB, b.endPointA)) {
			return true;
		} else {
			return false;
		}
	...
	public static void main(String[] args) throws IOException {
		...
		while (edgeNum < edges.length) {
			...
			while (edgeNum < edges.length
					&& (i == 0 || edges[edgeNum].length == SLE[0].length)) {
				if (inSameSet(edges[edgeNum].endPointA,
						edges[edgeNum].endPointB)) {
					edgeNum++;
					continue;
				}
				SLE[i++] = edges[edgeNum++];
			...
			if (i == 1) {
				...
				if (numComponents == 2) {
					...
					distance += length;
						...
						&& (!connectSameComponents(SLE[0], SLE[1]))
						&& (!connectSameComponents(SLE[1], SLE[2]))
						&& (!connectSameComponents(SLE[0], SLE[2]))) {
					// triangle shape
					merge(SLE[0].endPointA, SLE[0].endPointB);
					merge(SLE[1].endPointA, SLE[1].endPointB);
					numWays *= 3;
		...
		System.out.println(distance + " " + numWays);
