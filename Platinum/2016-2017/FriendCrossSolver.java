...
class SlowSolver {
	...
	// consulted Internet for a java collection that could do o(log n) count less with o(log n) access; didn't find one

	static int runInstance() throws IOException {
		return new SlowSolver().execute();
	}
	...
	int execute() throws IOException {
		...
		int count = 0;
		for (int i = 1; i <= numE; i++) {
			for (int inRange = 1; inRange < i - K; inRange++) {
				if (aLoc[i] <= aLoc[inRange] && bLoc[inRange] <= bLoc[i]
						|| aLoc[inRange] <= aLoc[i] && bLoc[i] <= bLoc[inRange]) {
					count++;
				}
			}
		}
