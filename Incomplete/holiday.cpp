...
// divide and conquer as endpoint monotonically increases with cost
void computeRange(int lCost, int hCost, int lEnd, int rEnd) {
    if (lCost > hCost) return;
    ps(lEnd, rEnd);
...
//    ps(bEnd);
    ...
    if(lEnd == rEnd){
        for(int cCost = lCost; cCost <= hCost; cCost++){
            ans[cCost] = bVal;
        }
//        return;
    }

    computeRange(lCost, midCost - 1, lEnd, bEnd);
    computeRange(midCost + 1, hCost, bEnd, rEnd);
}
