...
public class YinAndYangSolver {

	public static void main(String[] args) throws IOException {
		long startTime = System.currentTimeMillis();
		new YinAndYangSolver().execute();
	...
	void modStats(int cV, int p, int cD, int delta) {
		if (ENC[cD + offset] > 0) {
	...
	void countEnd(int cV, int p, int cD, boolean split) {
		if (split && cD == 0) {
			ans++;
		}
		if (cD == 0) {
	...
	void updAns(int cV, int p, int cD) {
