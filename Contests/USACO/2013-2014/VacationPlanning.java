...
public class VacationPlanning {
	...
	void execute() throws IOException {
		...
		for (int i = 0; i < numEdges; i++) {
			...
			aList.get(s).add(new Pair(e, d));
			rAList.get(e).add(new Pair(s, d));
		}
		...
		for (int i = 0; i < numHubs; i++) {
			...
			queue.add(new State(currentHub, 0));
			...
			while (!queue.isEmpty()) {
				...
				for (Pair neighborP : rAList.get(cV)) {
					...
					if (cost < distance[neighbor]) {
						...
						queue.add(new State(neighbor, cost));
					}
				}
			}
		...
		for (int currentRequest = 0; currentRequest < numRequests; currentRequest++) {
			...
			for (int currentHub = 0; currentHub < numHubs; currentHub++) {
				if (toDistance[currentHub][start] == Long.MAX_VALUE
						|| fromDistance[currentHub][end] == Long.MAX_VALUE) {
					continue;
				}
