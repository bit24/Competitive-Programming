...
public class Div1_373D {
	...
	TreeSet<Long> rtHashes = new TreeSet<Long>();
	...
	void calculate(int cV) {
		...
		if (numI[cV] == aList[cV].size() - 1) {
			...
			long tHash = (hash * BASE + ')') % MOD;
			long tHash2 = (hash2 * BASE2 + ')') % MOD;

			long fHash = (tHash2 << 32) ^ tHash;
			if (aList[cV].size() < 4) {
				rtHashes.add(fHash);
			}
		}
